...
def w_perp(x, px, sini, paascnode, Pb, PMRA, PMDEC, w):
    """calculate delta using e1dot and e2dot
    """
    Mtot = m1 + m2
    nb = 2. * np.pi / Pb
    ...
    return linalg.norm(w_orb.T * A_ref) , linalg.norm(w_orb.T*B_ref)
...
Wr = np.linspace(-3000.e5, 3000.e5, num=120)
X = []
Y = []
#def calc(w):
...
def calc(w):
    #return  calcalpha3(F0, a, PX, SINI, PAASCNODE, PMRA, PMDEC, M1, M2, PB, ECC, OM, E1DOT, E2DOT, w, alpha_1).flatten()
    return  w_perp(a, PX, SINI, PAASCNODE, PB, PMRA, PMDEC, w)
